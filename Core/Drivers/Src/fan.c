...
void fan_init(void)
{
    ...
    f.present = (HAL_I2C_IsDeviceReady(&hi2c2, (0x2F<<1), 3, 50) == HAL_OK);
    f.pwm = 0;
    f.rpm = 0;
    f.fault = false;
}
...
void fan_set_pwm(uint8_t pwm)
{
    ...
    data[1] = pwm;
    HAL_I2C_Master_Transmit(&hi2c2, (0x2F<<1), data, 2, 100);
}
...
void fan_poll(void)
{
    ...
    uint8_t reg = 0x00;
    uint8_t buf[2] = {0};
    if (HAL_I2C_Master_Transmit(&hi2c2, (0x2F<<1), &reg, 1, 100) != HAL_OK ||
        HAL_I2C_Master_Receive(&hi2c2, (0x2F<<1), buf, 2, 100) != HAL_OK) {
        f.fault = true;
        f.rpm = 0;
        return;
    }
